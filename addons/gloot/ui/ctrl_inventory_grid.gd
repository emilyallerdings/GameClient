...
signal item_moved_from_non_grid(item)
...
func _on_item_drop(zone: CtrlDropZone, drop_position: Vector2, ctrl_inventory_item: CtrlInventoryItemRect) -> void:
	...
	print("item dropped")
	item.new_pos = get_field_coords(drop_position)
	
	item.item_dropped_sig.emit()
...
func _on_dragable_dropped(dragable: CtrlDragable, drop_position: Vector2) -> void:
	...
	if inventory.connected_inventory && inventory.connected_inventory.inventory_id == item.get_property("connected_inv"):
		return
...
func _handle_item_transfer(item: InventoryItem, drop_position: Vector2) -> void:
	...
	if !(item.get_inventory() is InventoryGrid):
		var source_inventory = item.get_inventory()
		var field_coords = get_field_coords(drop_position + (field_dimensions / 2))
		
		source_inventory.transfer(item,inventory)
		inventory.move_item_to(item, field_coords)
		
		item.new_pos = field_coords
		item.item_dropped_sig.emit()
		#source_inventory.remove_item(item)
		#var new_item = inventory.create_and_add_item_at(item.prototype_id, field_coords)
		#if (new_item):
		#	item_moved_from_non_grid.emit(new_item)
		#	return
		return
